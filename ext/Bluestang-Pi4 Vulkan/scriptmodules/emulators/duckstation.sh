...
function depends_duckstation() {
    local depends=(cmake libsdl2-dev pkg-config libevdev-dev git ninja-build libgbm-dev libdrm-dev)
    getDepends "${depends[@]}"
}
...
function configure_duckstation() {
    ...
    addEmulator 0 "$md_id" "psx" "$md_inst/duckstation-nogui -portable -- %ROM%"
    addEmulator 0 "$md_id-setup" "psx" "$md_inst/duckstation-nogui -portable"
    addSystem "psx"
    ...
    mkUserDir "$home/.local/share/duckstation/bios"
    moveConfigDir "$home/.local/share/duckstation/bios" "$biosdir"
    moveConfigDir "$home/.local/share/duckstation/memcards" "$romdir/psx/saves"

    cat >"$romdir/psx/setup.sh" <<_EOF_
#!/bin/bash

"$md_inst/duckstation-nogui -portable"
_EOF_

    chown -R $user:$user "$romdir/psx/setup.sh"
