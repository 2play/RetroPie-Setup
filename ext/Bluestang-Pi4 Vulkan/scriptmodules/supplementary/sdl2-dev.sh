...
function build_sdl2-dev() {
    ...
    if isPlatform "rpi"; then
        isPlatform "rpi4" && conf_flags+=("--disable-video-rpi")
    else
        conf_flags+=("--enable-video-rpi")
    fi
    isPlatform "kms" || isPlatform "rpi" && conf_flags+=("--enable-video-kmsdrm" "--disable-video-wayland")
...
function install_sdl2-dev() {
    ...
    if ! dpkg -i libsdl2-2.0-0_$(get_pkg_ver_sdl2-dev)_$(get_arch_sdl2-dev).deb libsdl2-dev_$(get_pkg_ver_sdl2-dev)_$(get_arch_sdl2-dev).deb; then
